      ...
      <div class="col-xs-12">
        <div id="all-users-table-container" class="table-responsive">
          <table class="table table-striped table-hover table-condensed">
            ...
            <tbody>
              @if (isset($deletedPlaces) && !empty($deletedPlaces) && $deletedPlaces->count() > 0)
                @foreach ($deletedPlaces as $place => $details)
                  ...
                  <tr class="{{ $placeStatusClass }}">
                    ...
                    <td>
                      ...
                      @if ($details->place_status == "3")
                        <form action="{{route('admin.place.delete', ["hard", $details->place_id_hash])}}" method="post" class="delete-form hard-delete-form">
                          @csrf
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function(){
      $(".hard-delete-form").on("submit", function(){
        var deleteConfirm = confirm("Are you sure you wish to permantely delete this place? The changes cannot be reversed!");
